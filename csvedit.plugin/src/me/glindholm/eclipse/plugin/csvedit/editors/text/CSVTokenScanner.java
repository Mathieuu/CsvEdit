 ...
 */
package me.glindholm.eclipse.plugin.csvedit.editors.text;
...
import me.glindholm.eclipse.plugin.csvedit.editors.text.CSVToken.CSVTokenType;
...
public class CSVTokenScanner implements ITokenScanner {
     ...
     */
    public CSVTokenScanner(final char separator) {
    ...
    public IToken nextToken() {
        ...
        final int startOffset = m_currentOffset;
        int length = 0;
        try {
            // Get line information
            final IRegion lineRegion = m_document.getLineInformationOfOffset(m_currentOffset);
            final int lineMinOffset = lineRegion.getOffset();
            final int lineMaxOffset = lineMinOffset + lineRegion.getLength();
            // Read char
            final char current = m_document.getChar(m_currentOffset++);
            length++;
            ...
            if (current == '\n') {
                ...
                result = new CSVToken(CSVTokenType.SEPARATOR, m_currentColumn++);
                m_tokenOffset = startOffset;
                m_tokenLength = length;
                ...
                while (scan) {
                    ...
                    if (m_currentOffset >= lineMaxOffset) {
                        final CSVTokenType type = m_currentColumn % 2 == 0 ? CSVTokenType.ODD_COLUMN : CSVTokenType.EVEN_COLUMN;
                        ...
                        final char next = m_document.getChar(m_currentOffset);
                        if (next == m_delimiter) {
                            final CSVTokenType type = m_currentColumn % 2 == 0 ? CSVTokenType.ODD_COLUMN : CSVTokenType.EVEN_COLUMN;
                            result = new CSVToken(type, m_currentColumn);
