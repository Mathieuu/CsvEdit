...
public abstract class AbstractCSVFile implements IRowChangesListener {
    ...
    public String getTextRepresentation() {
        ...
        try {
            ...
            /*
             * if (isFirstLineHeader() && header.size() > 0) { String[] headerArray = new
             * String[header.size()]; for (int i=0; i<header.size(); i++) { headerArray[i] =
             * header.get(i); } clw.writeRecord(headerArray); }
             */
            for (final CSVRow row : rows) {
                if (row.isCommentLine()) {
